...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    document.setTitle("Welcome to <span class='text-blue-500 font-semibold'>BMKG.XYZ</span>");
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/v2ray")) {
        ...
        } else if (apiPath.startsWith("/v2ray")) {
...
class Document {
  ...
  buildCountryFlag() {
    ...
    for (const flag of new Set(flagList)) {
      flagElement += `<a href="/v2ray?country=${flag}${
