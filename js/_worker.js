...
const serviceName = "ns1"; // Ganti dengan nama workers kalian
...
function getAllConfig(request, hostName, proxyList, page = 0) {
  ...
  try {
    ...
    // Build pagination
    document.addPageButton("Prev", `/view/${page > 0 ? page - 1 : 0}`, page > 0 ? false : true);
    document.addPageButton("Next", `/view/${page + 1}`, page < Math.floor(proxyList.length / 10) ? false : true);
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/view")) {
        const page = url.pathname.match(/^\/view\/(\d+)$/);
        const pageIndex = parseInt(page ? page[1] : "0");
        const hostname = request.headers.get("Host");

        // Queries
        const countrySelect = url.searchParams.get("negara")?.split(",");
        ...
        } else if (apiPath.startsWith("/view")) {
          const filterCC = url.searchParams.get("negara")?.split(",") || [];
...
class Document {
  ...
  buildCountryFlag() {
    ...
    for (const flag of new Set(flagList)) {
      flagElement += `<a href="/view?negara=${flag}${
